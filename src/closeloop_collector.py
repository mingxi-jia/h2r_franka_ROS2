...
class SynchronizedDatasetCollector(Node):
    def __init__(self):
        ...
        self.ts = ApproximateTimeSynchronizer(
            [self.rgb_sub, self.depth_sub, self.action_sub, self.pose_sub, self.franka_joint_sub, self.gripper_action_sub],
            queue_size=20,          
            slop=self.time_diff
        )
    ...
    def sync_callback(self, rgb: Image, depth: Image, action_stamped:TwistStamped, pose_stamped:PoseStamped, franka_joint_states:JointState, gripper_action: Joy):
        ...
        if self.saving_flag:
            ...
            step = dict()
            step["seconds"] = rgb.header.stamp.sec
            step["nanoseconds"] = rgb.header.stamp.nanosec
            step["rgb"] = rgb_data
            step["depth"] = depth_data
