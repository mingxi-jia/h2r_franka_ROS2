...
class FCNInHandDynamicFilterU(nn.Module, UBase):
    def __init__(self, n_input_channel=1, n_primitives=1, patch_shape=(1, 24, 24), domain_shape=(1, 100, 100)):
        super().__init__()
        UBase.__init__(self, n_input_channel)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1):
        ...
        self.dilation = dilation
        self.downsample = downsample
        middle_planes = int(planes/4)

        self.conv1 = conv1x1(inplanes, middle_planes, stride, dilation=dilation)
        # self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(middle_planes, middle_planes, dilation=dilation)
        # self.bn2 = nn.BatchNorm2d(planes)

        self.conv3 = conv1x1(middle_planes, planes, dilation=dilation)
...
class ResNet34(BaseModel):
    def __init__(self, n_input_channel=1, n_primitives=1, patch_shape=(1, 24, 24), domain_shape=(1, 100, 100)):
        ...
        self._encoder = nn.Sequential(
            OrderedDict(
                [
                    ("enc-conv1",
                     nn.Conv2d(self._in_channels, 32, kernel_size=7, stride=1, padding=3, bias=False)),
                    ("enc-relu1",
                     nn.ReLU(inplace=True)),
                    ("enc-pool2",
                    ...
                    ("enc-pool3",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn3-1",
                     BasicBlock(32, 64, stride=1, dilation=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(32, 64, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn3-2",
                     BasicBlock(64, 64, dilation=1)),
                    ("enc-resn3-3",
                     BasicBlock(64, 64, dilation=1)),
                    ("enc-resn3-4",
                     BasicBlock(64, 64, dilation=1)),
                    ("enc-pool4",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn4-1",
                     BasicBlock(64, 128, dilation=1, stride=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(64, 128, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn4-2",
                     BasicBlock(128, 128, dilation=1)),
                    ("enc-resn4-3",
                     BasicBlock(128, 128, dilation=1)),
                    ("enc-resn4-4",
                     BasicBlock(128, 128, dilation=1)),
                    ...
                    ("enc-pool5",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn5-1",
                     BasicBlock(128, 256, dilation=1, stride=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(128, 256, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn5-2",
                     BasicBlock(256, 256, dilation=1)),
                    ("enc-resn5-3",
                     BasicBlock(256, 256, dilation=1)),
                ]
            )
            ...
            OrderedDict(
                [
                    ...
                    ("dec-upsm2",
                     Interpolate(scale_factor=2, mode="bilinear", align_corners=True)),
                    ("dec-resn2-1",
                     BasicBlock(256, 128, dilation=1,
                                downsample=nn.Sequential(nn.Conv2d(256, 128, kernel_size=1, bias=False)))),
                    ("dec-resn2-2",
                     BasicBlock(128, 128, dilation=1)),
                    ("dec-resn2-3",
                     BasicBlock(128, 128, dilation=1)),
                    ...
                    ("dec-upsm3",
                     Interpolate(scale_factor=2, mode="bilinear", align_corners=True)),
                    ("dec-resn3-1",
                     BasicBlock(128, 64, dilation=1,
                                downsample=nn.Sequential(nn.Conv2d(128, 64, kernel_size=1, bias=False)))),
                    ("dec-resn3-2",
                     BasicBlock(64, 64, dilation=1)),
                    ("dec-resn3-3",
                     BasicBlock(64, 64, dilation=1)),
                    ...
                    ("dec-upsm4",
                     Interpolate(scale_factor=2, mode="bilinear", align_corners=True)),
                    ("dec-resn4-1",
                     BasicBlock(64, 32, dilation=1,
                                downsample=nn.Sequential(nn.Conv2d(64, 32, kernel_size=1, bias=False)))),
                    ("dec-resn4-2",
                     BasicBlock(32, 32, dilation=1)),
                    ("dec-resn4-3",
                     BasicBlock(32, 32, dilation=1)),
    ...
    def forward(self, obs, in_hand):
        enc_out = self._encoder(obs)
        in_hand_out = self.in_hand_conv(in_hand)
        dec_in = self.conv_cat_in_hand(torch.cat((enc_out, in_hand_out), dim=1))
        feature_map = self._decoder(dec_in)

        place_q_values = self.place_q_values(feature_map)
        pick_q_values = self.pick_q_values(feature_map)
        q_values = torch.cat((pick_q_values, place_q_values), dim=1)

        return q_values

class Res34Shared96(ResNet34):
    def __init__(self, n_input_channel=1, n_primitives=1, patch_shape=(1, 24, 24), domain_shape=(1, 100, 100)):
        super().__init__(n_input_channel, n_primitives, patch_shape, domain_shape)
        self.in_hand_conv = InHandConv6(patch_shape)
        # self.feature_map_down = nn.Conv2d(512, 256, kernel_size=1, stride=1, bias=False)
            ...
            OrderedDict(
                [
                    ...
                    ("enc-pool2",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn2-1",
                     Bottleneck(32, 128, dilation=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(32, 128, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn2-2",
                     Bottleneck(128, 128, dilation=1)),
                    ("enc-resn2-3",
                     Bottleneck(128, 128, dilation=1)),
                    ("enc-pool3",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn3-1",
                     Bottleneck(128, 256, stride=1, dilation=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(128, 256, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn3-2",
                     Bottleneck(256, 256, dilation=1)),
                    ("enc-resn3-3",
                     Bottleneck(256, 256, dilation=1)),
                    ("enc-resn3-4",
                     Bottleneck(256, 256, dilation=1)),
                    ("enc-pool4",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn4-1",
                     Bottleneck(256, 512, dilation=1, stride=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(256, 512, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn4-2",
                     Bottleneck(512, 512, dilation=1)),
                    ("enc-resn4-3",
                     Bottleneck(512, 512, dilation=1)),
                    ("enc-resn4-4",
                     Bottleneck(512, 512, dilation=1)),
                    ...
                    ("enc-pool5",
                     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
                    ("enc-resn5-1",
                     Bottleneck(512, 1024, dilation=1, stride=1,
                                downsample=nn.Sequential(
                                    nn.Conv2d(512, 1024, kernel_size=1, stride=1, bias=False),
                                ))),
                    ("enc-resn5-2",
                     Bottleneck(1024, 1024, dilation=1)),
                    ("enc-resn5-3",
                     Bottleneck(1024, 1024, dilation=1)),
                ]
            )
            ...
            OrderedDict(
                [
                    ...
                    ("dec-upsm2",
                     Interpolate(scale_factor=2, mode="bilinear", align_corners=True)),
                    ("dec-resn2-1",
                     Bottleneck(1024, 512, dilation=1,
                                downsample=nn.Sequential(nn.Conv2d(1024, 512, kernel_size=1, bias=False)))),
                    ("dec-resn2-2",
                     Bottleneck(512, 512, dilation=1)),
                    ("dec-resn2-3",
                     Bottleneck(512, 512, dilation=1)),
                    ...
                    ("dec-upsm3",
                     Interpolate(scale_factor=2, mode="bilinear", align_corners=True)),
                    ("dec-resn3-1",
                     Bottleneck(512, 256, dilation=1,
                                downsample=nn.Sequential(nn.Conv2d(512, 256, kernel_size=1, bias=False)))),
                    ("dec-resn3-2",
                     Bottleneck(256, 256, dilation=1)),
                    ("dec-resn3-3",
                     Bottleneck(256, 256, dilation=1)),
...
class Res101Shared96(ResNet101):
    def __init__(self, n_input_channel=1, n_primitives=1, patch_shape=(1, 24, 24), domain_shape=(1, 100, 100)):
        super().__init__(n_input_channel, n_primitives, patch_shape, domain_shape)
        self.in_hand_conv = InHandConv6(patch_shape)
        self.feature_map_down = nn.Conv2d(1024, 256, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, obs, in_hand):
        ...
        return q_values, self.feature_map_down(dec_in)
...
class ResUCat96Sig(ResUCat):
    def __init__(self, n_input_channel=1, n_primitives=1, patch_shape=(1, 24, 24), domain_shape=(1, 100, 100)):
        super().__init__(n_input_channel, n_primitives, patch_shape, domain_shape)
        self.place_q_values = None
        self.in_hand_conv = InHandConv6(patch_shape)
    ...
    def forward(self, obs, in_hand):
        ...
        return torch.sigmoid(pick_q_values)
        # return torch.sigmoid(super().forward(obs, in_hand))


class CNNResShared96(nn.Module):
    def __init__(self, image_shape, n_outputs):
        super().__init__()
        self.patch_conv = InHandConv6(image_shape)
        conv_out_size = self._getConvOut(image_shape)
        self.fc1 = nn.Linear(conv_out_size, 1024)
        self.fc2 = nn.Linear(1024, n_outputs)
...
class CNNRes(nn.Module):
    def __init__(self, image_shape, n_outputs):
        ...
        self.conv = nn.Sequential(OrderedDict([
            ('cnn_conv1', nn.Conv2d(image_shape[0], 32, kernel_size=3, padding=1)),
            ('cnn_relu1', nn.ReLU(inplace=True)),
            (
                'cnn_res2',
                BasicBlock(
...
class CNNResSig(CNNRes):
    def __init__(self, image_shape, n_outputs):
        super().__init__(image_shape, n_outputs)

    def forward(self, x):
        return torch.sigmoid(super().forward(x))

class CNNResI(nn.Module):
    def __init__(self, i_shape, patch_shape, n_outputs):
        super().__init__()
        self.obs_conv = nn.Sequential(OrderedDict([
            ('obs_conv1', nn.Conv2d(i_shape[0], 32, kernel_size=3, padding=1)),
            ('obs_relu1', nn.ReLU(inplace=True)),
            ('obs_pool1', nn.MaxPool2d(2)),
            ('obs_res2',
                BasicBlock(
                    32, 64,
                    downsample=nn.Sequential(
            ...
            ('obs_pool2', nn.MaxPool2d(2)),
            ('obs_res3',BasicBlock(64, 64)),
            ('obs_pool3', nn.MaxPool2d(2)),
            ('obs_res4', BasicBlock(64, 64)),
        ]))

        self.patch_conv = nn.Sequential(OrderedDict([
            ('patch_conv1', nn.Conv2d(patch_shape[0], 32, kernel_size=3, padding=1)),
            ('patch_relu1', nn.ReLU(inplace=True)),
            ('patch_res2',
                BasicBlock(
                    32, 64,
                    downsample=nn.Sequential(
            ...
            ('patch_pool2', nn.MaxPool2d(2)),
            ('patch_res3', BasicBlock(64, 64)),
        ]))

        conv_out_size = self._getConvOut(i_shape, patch_shape)
        self.fc1 = nn.Linear(conv_out_size, 1024)
        self.fc2 = nn.Linear(1024, n_outputs)
    ...
    def _getConvOut(self, obs_shape, patch_shape):
        o1 = self.obs_conv(torch.zeros(1, *obs_shape))
        o2 = self.patch_conv(torch.zeros(1, *patch_shape))
        return int(np.prod(o1.size()))+int(np.prod(o2.size()))

    def forward(self, obs, patch):
        obs_conv_out = self.obs_conv(obs)
        obs_conv_out = obs_conv_out.view(obs.size(0), -1)
        patch_conv_out = self.patch_conv(patch)
        patch_conv_out = patch_conv_out.view(patch.size(0), -1)

        x = torch.cat((obs_conv_out, patch_conv_out), dim=1)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)
        return x
...
class CNNI(nn.Module):
    def __init__(self, i_shape, patch_shape, n_outputs):
        ...
        self.obs_conv = nn.Sequential(OrderedDict([
            ...
            ('obs_relu1', nn.ReLU(inplace=True)),
            ('obs_pool1', nn.MaxPool2d(2)),
            ('obs_conv2', nn.Conv2d(32, 64, kernel_size=3, padding=1)),
            ('obs_relu2', nn.ReLU(inplace=True)),
            ('obs_pool2', nn.MaxPool2d(2)),
            ('obs_conv3', nn.Conv2d(64, 64, kernel_size=3, padding=1)),
            ('obs_relu3', nn.ReLU(inplace=True)),
            ('obs_pool3', nn.MaxPool2d(2)),
            ('obs_conv4', nn.Conv2d(64, 64, kernel_size=3, padding=1)),
            ('obs_relu4', nn.ReLU(inplace=True)),
            ('obs_pool4', nn.MaxPool2d(2))
        ]))

        self.patch_conv = nn.Sequential(OrderedDict([
            ('patch_conv1', nn.Conv2d(patch_shape[0], 32, kernel_size=3, padding=1)),
            ('patch_relu1', nn.ReLU(inplace=True)),
            ('patch_conv2', nn.Conv2d(32, 64, kernel_size=3, padding=1)),
            ('patch_relu2', nn.ReLU(inplace=True)),
            ('patch_pool2', nn.MaxPool2d(2)),
            ('patch_conv3', nn.Conv2d(64, 64, kernel_size=3, padding=1)),
            ('patch_relu3', nn.ReLU(inplace=True)),
            ('patch_pool3', nn.MaxPool2d(2)),
        ]))
