...
from droid.subprocess_utils import run_threaded_command
from droid.transformations import add_angles, euler_to_quat, quat_diff, quat_to_euler, rmat_to_quat
...
class VRPolicy: # need to find out where we run this VRPolicy in the original code
    def __init__(
        self,
        right_controller: bool = True,
        max_lin_vel: float = 0.5,
        max_rot_vel: float = 0.8,
        ...
        # rmat_reorder: list = [1, -2, -3, 4],
    ):
        self.oculus_reader = OculusReader()  #investigate
        ...
        # global_to_env_euler = [0., 0., 0.] # order xyz
        # global_to_env_rot = R.from_euler("xyz", global_to_env_euler).as_matrix()
        # global_to_env_mat = np.eye(4)
        # global_to_env_mat[:3, :3] = global_to_env_rot
        # self.global_to_env_mat = global_to_env_mat
    ...
    def _update_internal_state(self, num_wait_sec=5, hz=50):
        ...
        while True:
            ...
            # self._state["movement_enabled"] = buttons["rightTrig" if self.controller_id == "r" else "leftTrig"][0] > 0.9
    ...
    def _process_reading(self): 
        ...
        #     print("No poses available. Using the last known pose for stability.")
        #     if self.vr_state:  # Ensure we have a valid previous state
        #         return  # Continue using the last known state
        #     else:
        #         print("No previous pose available. Skipping processing entirely.")
        #         return
        ...
        self.vr_state = {"pos": vr_pos, "quat": vr_quat, "gripper": vr_gripper, "movement_enabled": self._state["movement_enabled"]}
    ...
    def _calculate_action(self, state_dict, include_info=False):
        ...
        if self.update_sensor:
            # what should we do if there are no poses?
        ...
        # if not self.vr_state["movement_enabled"]:
        #     action = np.zeros_like(action)
        if (abs(action) < 0.001).any():
            action = np.zeros_like(action)
